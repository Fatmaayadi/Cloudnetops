...
export default function EC2() {
  ...
  useEffect(() => {
    load();
    const interval = setInterval(load, 10000); // Refresh toutes les 10s
  ...
  async function terminate(e) {
    ...
    try {
      ...
      // Retirer l'instance terminée immédiatement
      setInstances(prev =>
        prev.filter(i => i.instanceId.trim() !== terminateId.trim())
      );
      setSelected(prev => (prev === terminateId ? '' : prev));
      ...
      <div className="grid fade-in">
        ...
        <div className="card interactive">
          ...
          <div className="card-body">
            {instances.filter(i => i.state !== 'terminated').length === 0 ? (
